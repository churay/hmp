...
extern "C" bool32_t render( const demo::state_t* pState, const demo::input_t* pInput, const demo::output_t* pOutput ) {
    ...
    const static auto hsv2rgb = [] ( const color4f32_t& hsv ) {
        // TODO(JRC): Write this code.
        return hsv;
    };

    const static auto colorf2coloru = [] ( const color4f32_t& cf ) {
        color4u8_t cu;
        cu.x = std::floor( cf.x >= 1.0 ? 255 : cf.x * 256.0 );
        cu.y = std::floor( cf.y >= 1.0 ? 255 : cf.y * 256.0 );
        cu.z = std::floor( cf.z >= 1.0 ? 255 : cf.z * 256.0 );
        cu.w = std::floor( cf.w >= 1.0 ? 255 : cf.w * 256.0 );
        return cu;
    };
